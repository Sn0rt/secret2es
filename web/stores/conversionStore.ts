...
interface ConversionState {
  ...
  updateRefreshPolicy: (refreshPolicy: string) => void;
  updateRefreshInterval: (refreshInterval: string) => void;
...
const initialState = {
  ...
  formData: {
    ...
    refreshPolicy: DEFAULT_VALUES.refreshPolicy,
    refreshInterval: DEFAULT_VALUES.refreshInterval,
  },
  isFormValid: false,
...
export const useConversionStore = create<ConversionState>()(
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      updateRefreshPolicy: (refreshPolicy: string) => {
        set((state) => {
          const newFormData = { ...state.formData, refreshPolicy };
          return {
            formData: newFormData,
            isFormValid: validateFormData(newFormData.storeName, state.yamlError),
          };
      ...
      updateRefreshInterval: (refreshInterval: string) => {
        set((state) => {
          const newFormData = { ...state.formData, refreshInterval };
          return {
            formData: newFormData,
            isFormValid: validateFormData(newFormData.storeName, state.yamlError),
          };
      ...
      convertYaml: async () => {
        ...
        try {
          ...
          const request: ConversionRequest = {
            ...
            refreshPolicy: state.formData.refreshPolicy,
            refreshInterval: state.formData.refreshPolicy === 'Periodic' ? state.formData.refreshInterval : undefined,
